function f = lab5_2()
    ...
    for i = 1:folds
        ...
        [error, prototypes] = lab5_2_lvq1(epochs, P_train, train_data, K, n);
        training_error(i) = (error(end) / P_train * 100);
        classification_error(i) = (test_error(prototypes, test_data, K) / P_test * 100);
    end
    ...
    saveas(fig,"lab5_2_2.png");

end

function error = test_error(prototypes, test_data, K)
    prototypes_A = prototypes(1:K(1),:);
    prototypes_B = prototypes(K(1)+1:end,:);
    error = 0;
    for i = 1:size(test_data, 1)
        dist_to_A = min(pdist2(test_data(i,:), prototypes_A));
        dist_to_B = min(pdist2(test_data(i,:), prototypes_B));
